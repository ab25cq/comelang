...
void task1_func(){
unsigned int t_263;
memset(&t_263, 0, sizeof(unsigned int));
    gpio_init(LED_PIN);
    gpio_set_dir(LED_PIN,1);
    while(1) {
        gpio_put(LED_PIN,1);
        piccolo_sleep(&t_263,1000);
        gpio_put(LED_PIN,0);
        piccolo_sleep(&t_263,1000);
    }
}
...
int is_prime(unsigned int n){
unsigned int p_264;
memset(&p_264, 0, sizeof(unsigned int));
    if(    !(n&1)||n<2) {
        return n==2;
    }
    for(    p_264=3;    p_264<=n/p_264;    p_264+=2    ){
        if(        !(n%p_264)) {
            return 0;
        }
    }
...
unsigned int t_265;
int p_266;
memset(&t_265, 0, sizeof(unsigned int));
memset(&p_266, 0, sizeof(int));
    printf("task2: Created!\n");
    while(1) {
        p_266=to_ms_since_boot(get_absolute_time());
        if(        is_prime(p_266)==1) {
            printf("%d is prime!\n",p_266);
        }
        piccolo_yield();
