...
impl list <T>
{
    ...
    list<T>*% merge_list_default(list<T>* left, list<T>* right) {
        auto result = new list<T>.initialize();
    ...
    list<T>*% merge_sort_default(list<T>* self) {
        if(self.head == null) {
            return clone self;
        }
        if(self.head!.next == null) {
        ...
        return list1.merge_sort_default().merge_list_default( list2.merge_sort_default());
    }
    list<T>*% sort_default(list<T>* self) {
        return self.merge_sort_default();
    }
...
inline smart_pointer* smart_pointer*::operator_add(smart_pointer* self, int value)
{
    self.p+=value;
    
    if(self.p > self.end) {
        fprintf(stderr, "%s %d: out of range of smart pointer\n", __inline_caller_sname__, __inline_caller_sline__);
        exit(1);
    }
...
inline smart_pointer* smart_pointer*::operator_sub(smart_pointer* self, int value)
{
    self.p-=value;
...
inline char smart_pointer*::operator_derefference(smart_pointer* self)
{
    char* p = self.p;
    return *p;
}
...
impl buffer*
{
    ...
    smart_pointer*% to_pointer(buffer* self);
}
