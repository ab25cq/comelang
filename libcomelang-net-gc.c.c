...
int opt_58;
struct sockaddr_in address_59;
int addrlen_60;
int new_socket_61;
_Bool break__62;
memset(&address_59, 0, sizeof(struct sockaddr_in));
    if((_and_conditional2=(sock_57=socket(socket_family,socket_type,protocol))),    _and_conditional2 == 0) {
    ...
    if(reuse) {
        opt_58=1;
        if(setsockopt(sock_57,1,2,&opt_58,sizeof(opt_58))) {
            perror("setsockopt failed");
            close(sock_57);
            exit(1);
        }
    }
    address_59.sin_family=2;
    address_59.sin_addr.s_addr=((unsigned int)0);
    address_59.sin_port=htons(port);
    addrlen_60=sizeof(address_59);
    if(bind(sock_57,(struct sockaddr*)&address_59,sizeof(address_59))<0) {
        perror("Unable to bind");
        exit(1);
    }
    if(listen(sock_57,3)<0) {
        perror("Unable to listen");
        exit(1);
    }
    while(1) {
        new_socket_61=accept(sock_57,(struct sockaddr*)&address_59,(unsigned int*)&addrlen_60);
        break__62=(_Bool)0;
        block(parent,new_socket_61,&break__62);
        close(new_socket_61);
        if(break__62) {
            break;
        }
...
int httpsd_socket(int port, _Bool reuse, void* parent, void (*block)(void*,struct ssl_st*,_Bool*)){
int sock_63;
struct ssl_ctx_st* ctx_64;
const struct ssl_method_st* method_65;
int opt_66;
struct sockaddr_in addr_67;
struct sockaddr_in addr_68;
unsigned int len_69;
int client_70;
struct ssl_st* ssl_71;
_Bool break__72;
memset(&sock_63, 0, sizeof(int));
ctx_64 = (void*)0;
method_65 = (void*)0;
memset(&addr_67, 0, sizeof(struct sockaddr_in));
memset(&addr_68, 0, sizeof(struct sockaddr_in));
    OPENSSL_init_ssl(2097152|2,((void*)0));
    OPENSSL_init_ssl(0,((void*)0));
    method_65=TLS_server_method();
    ctx_64=SSL_CTX_new(method_65);
    if(!ctx_64) {
        perror("Unable to create SSL context");
        ERR_print_errors_fp((stdout));
        exit(1);
    }
    if(SSL_CTX_use_certificate_file(ctx_64,"cert.pem",1)<=0) {
        ERR_print_errors_fp((stdout));
        exit(1);
    }
    if(SSL_CTX_use_PrivateKey_file(ctx_64,"key.pem",1)<=0) {
        ERR_print_errors_fp((stdout));
        exit(1);
    }
    sock_63=socket(2,1,0);
    if(sock_63<0) {
    ...
    if(reuse) {
        opt_66=1;
        if(setsockopt(sock_63,1,2|15,&opt_66,sizeof(opt_66))) {
            perror("setsockopt failed");
            close(sock_63);
            exit(1);
        }
    }
    addr_67.sin_family=2;
    addr_67.sin_port=htons(port);
    addr_67.sin_addr.s_addr=((unsigned int)0);
    if(bind(sock_63,(struct sockaddr*)&addr_67,sizeof(addr_67))<0) {
        perror("Unable to bind");
        exit(1);
    }
    if(listen(sock_63,1)<0) {
        perror("Unable to listen");
        exit(1);
    }
    while(1) {
        len_69=sizeof(addr_68);
        client_70=accept(sock_63,(struct sockaddr*)&addr_68,&len_69);
        if(client_70<0) {
            perror("Unable to accept");
            exit(1);
        }
        ssl_71=SSL_new(ctx_64);
        SSL_set_fd(ssl_71,client_70);
        if(SSL_accept(ssl_71)<=0) {
            ERR_print_errors_fp((stdout));
        }
        else {
            break__72=(_Bool)0;
            block(parent,ssl_71,&break__72);
            if(break__72) {
                break;
            }
        }
...
char* check_user_query_73;
_Bool _or_conditional1;
struct st_mysql_res* result_74;
...
_Bool _or_conditional4;
    ...
    check_user_query_73=xsprintf("SELECT COUNT(*) FROM mysql.user WHERE user = '\%s' AND host = '\%s'",charp_to_string(user),charp_to_string(host_name));
    if((_or_conditional1=(mysql_query(gComeMySQL,check_user_query_73))),    _or_conditional1 != 0) {
        finish_with_error();
    }
    if((_and_conditional4=(result_74=mysql_store_result(gComeMySQL))),    _and_conditional4 == 0) {
        finish_with_error();
    }
    row_75=mysql_fetch_row(result_74);
    user_exists_76=atoi(row_75[0]);
    mysql_free_result(result_74);
    if(user_exists_76==0) {
...
_Bool _or_conditional5;
    ...
    create_db_query_77=xsprintf("CREATE DATABASE IF NOT EXISTS \%s",charp_to_string(database_name));
    if((_or_conditional5=(mysql_query(gComeMySQL,create_db_query_77))),    _or_conditional5 != 0) {
        finish_with_error();
    }
    come_mysql_final();
}
...
struct st_mysql_res* res_78;
_Bool _and_conditional10;
int num_fields_79;
struct list$1list$1charphph* result_80;
char** row_81;
struct list$1charph* li_82;
int i_83;
...
row_81 = (void*)0;
    ...
    if((_and_conditional10=(res_78=mysql_store_result(gComeMySQL))),    _and_conditional10 == 0) {
        finish_with_error();
    }
    num_fields_79=mysql_num_fields(res_78);
    result_80=list$1list$1charphph_initialize((struct list$1list$1charphph*)come_calloc(1, sizeof(struct list$1list$1charphph)*(1), "libcomelang-net-gc.c", 371, "list$1list$1charphph"));
    while((row_81=mysql_fetch_row(res_78))!=((void*)0)) {
        li_82=list$1charph_initialize((struct list$1charph*)come_calloc(1, sizeof(struct list$1charph)*(1), "libcomelang-net-gc.c", 375, "list$1charph"));
        for(        i_83=0;        i_83<num_fields_79;        i_83++        ){
            if(row_81[i_83]) {
                list$1charph_add(li_82,__builtin_string(row_81[i_83]));
            }
        }
        list$1list$1charphph_add(result_80,li_82);
    }
    mysql_free_result(res_78);
    come_mysql_final();
    __result69__ = gComeFunResultObject = __result_obj__ = result_80;
    gComeFunResultObject = (void*)0;
    return __result69__;
}
...
static struct list$1list$1charphph* list$1list$1charphph_initialize(struct list$1list$1charphph* self){
void* __result_obj__=(void*)0;
struct list$1list$1charphph* __result65__;
    self->head=((void*)0);
    self->tail=((void*)0);
    self->len=0;
...
struct list$1charph* __result67__;
    if(self->len==0) {
        litem_84=(struct list_item$1charph*)come_calloc(1, sizeof(struct list_item$1charph)*(1), "./comelang.h", 151, "list_item$1charph");
        ...
        if(self->len==1) {
            litem_85=(struct list_item$1charph*)come_calloc(1, sizeof(struct list_item$1charph)*(1), "./comelang.h", 161, "list_item$1charph");
            ...
            litem_86=(struct list_item$1charph*)come_calloc(1, sizeof(struct list_item$1charph)*(1), "./comelang.h", 171, "list_item$1charph");
...
static struct list$1list$1charphph* list$1list$1charphph_add(struct list$1list$1charphph* self, struct list$1charph* item){
void* __result_obj__=(void*)0;
struct list_item$1list$1charphph* litem_87;
struct list_item$1list$1charphph* litem_88;
struct list_item$1list$1charphph* litem_89;
struct list$1list$1charphph* __result68__;
    if(self->len==0) {
        litem_87=(struct list_item$1list$1charphph*)come_calloc(1, sizeof(struct list_item$1list$1charphph)*(1), "./comelang.h", 151, "list_item$1list$1charphph");
        ...
        if(self->len==1) {
            litem_88=(struct list_item$1list$1charphph*)come_calloc(1, sizeof(struct list_item$1list$1charphph)*(1), "./comelang.h", 161, "list_item$1list$1charphph");
            ...
            litem_89=(struct list_item$1list$1charphph*)come_calloc(1, sizeof(struct list_item$1list$1charphph)*(1), "./comelang.h", 171, "list_item$1list$1charphph");
