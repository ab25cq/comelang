...
comelangはC++のようなCを拡張した言語ですがGoやRust, Swift, Kotlinのようなモダンな言語に備わった機能に絞ってC言語を拡張していて、実装したいソフトウェアを簡単に作れるように工夫されてます。データベースサーバーやwebサーバー、コンパイラやインタプリタどは驚くほど簡単に実装できます。私は自分自身でcomelangを使ってエディッタやシェル、インタプリタ、webサーバーやデータベースサーバーを作ってみましたが、非常に簡単に実装することができました。だいたい思いついてから2,3日で形にすることができました。私がC言語に熟練していることもあるとは思いますが、comelangにはGCとコレクションライブラリがあるため、煩わしいメモリ管理をしなくていいこともあると思います。

Cしか使えない環境でもcomelangは-Sオプションを使うとCソースを出力するため、Cが動く環境なら多少の工夫で動くはずです。まあ、そういう環境ではクロスコンパイルしてアプリを作成するため、C++が使えることも多いかもしれません。でも、そのような環境で使うコンパイラの選択肢としてcomelangを使うことも考えられるでしう。
...
どちらのエディッターもカスタマイズができますが、エディッターのカスタマイズは最小限にすことをお勧めします。
...
と工場のラインのように処理されます。comelangは実は正確にはコンパイラではなくC言語のソースファイルを出力するトランスパイラです。このC言語を出力する方法には利点がいくつかあります。一つはC言語が動くコンパイラさえあればcomelangが使えること、二つ目はセルフホスト（自分自身をコパイルする。comelangはcomelangで作られてます）が容易であること、C言語用のデバッガーが使えることです。
...
最初のintというのは64bitの数型で、まあ、普通に一般的な数字を扱う数値型と覚えればいいでしょう。これは関数の戻り値の型を定義しています。この場合main関数という関数の実行結果を数値型としています。main関数はint型を返さないといけないとされていて、main関数の戻り値がシェルの`$?`という環境変数に入れられるコマンドのステータスコードとされています。
...
(の後のint argc, char\*\* argvはメイン関数に渡される引数で、メイン関数の場合はコマドライン引数となります。例えば./a 1 2 3とした場合argcは4が入っていてargv[0]には./aという文字列が、argv[1]には1がargv[2]には2がargv[3]には3が入っています。注意すべきなのは最初のargvには./aというコマンド名自身が渡されている点です。
...
に戻りましょう。putsというのは引数の文字列を出力して改行コードを追加して出力します。改行コードはMac, Linuxでは\n(10)です。改行コードはchar型(8bit数）で数値にすると10のことです。コンピュータでは実は文字ですら数値に過ぎないため、このように改行コードも10がマッピングされています。要するにABCDEFGなどと人間が読める文字だけでなく特別な意味を持つ機能的な文字もchar型（8bit数値型)として扱われます。これはコントロール文字と呼ばれ、画面のクリアやベル音を出す、端末の出力を止めるなどの機能も特定の数字が割り当てられてます。
...
{}というのは関数の始まりと終わりを示すものです。PythonなどとはいC言語はフリーフォマットの言語のため、始まりと終わりを示す必要があります。
...
.hが拡張子のファイルはヘッダーファイルと呼ばれて、ライブラリを呼び出すときに\#includeると、そのライブラリを使うことができます。
...
メモリというのはコンピュータにある０と1の状態を持つ装置で、1つの番地に対して32bitのCPUなら２進数で32\*8桁、64bitのCPUなら64\*8桁の0と1の数値を格納することができます。
...
としますbは0xFFcd FF3bc ..などと64bitのCPUなら64bitの数値が入っています。
...
segmentation faultのバグはgdbやlldbを使ってバグがある箇所を特定します。gdbを使うためにはプロラムのコンパイル時に-gオプションをつけて、comelangなら-sオプションもつけてコンパイルしてください。コマンド名が./aならgdb ./aとしてgdbのコマンドラインに入りrとすれば、プログラムを実行できます。segmentaion faultが起これば、gdbのコマンドラインでbtとすればスタックフレームが見れます。（関数の呼び出し履歴とソースファイル番号）このようにしてsegmentation faultが起こったソースファイルの場所を特定できます。
...
に戻りまししょう。この場合aはメモリのどの部分に格納されるのでしょうか？それはスタック領域です。スタック領域というのは関数が呼ばれたときに作られますが、関数から脱出すると全部消されてしまいます。この領域はユーザーが意識することなく自動的に確保されるため便利です。関数の呼び出しのネストが大きくなって変数の確保が多数になっても、一旦関数から脱出するとメモリが枯渇することがありません。ただし関数が脱出すると自動的に解放されるということ以下のようなコードはC言語では実行時エラーとなります。
...
その不正なメモリに対して\*aとint型の大きさだけ読み込むと不正なメモリアクセスとなりま。(しかも、segmentation faultも起こらず実行されてしまう）
...
と呼び出します。もし、これを呼び出さなとメモリが残ったままとなり何度もこれが行われるとメモリは枯渇します。
...
comelangではこの問題は以下のように解しています。mallocは直接使わず
...
ほんとどの場合そのままincludeしても問題ありませんがまれにC言語のヘッダーフイル内のinline関数で(1,2,3)などと使われている場合があるためです。
...
Personはクラスで文字列型のname, 数値型のage, kMale, kFemaleのどちらかが入ったgenderというフィールを持っています。
...
`[]`がリストの表現となります。わかりやすいようにPythonと同じにしあります。
...
は、型安全なスマートポインタでchar型でメモリにアクセスしています。
スマートポインタは範囲外に達するとSegmentation faultを起こさず、エラーメッセージを表示します。
...
bufferは上記のようにすれば\0を含むバイトコードも扱うことができます例えばある構造体のポインタに代入すればその構造体のメモリ配列でメモリにアクセスできるでしょう。
...
ただし、サーバーアプリ、コンパイラ、インタプリタ、エディッターなどのサーバーで書くようなシステム系のプログラムではcomelangのようにデータのサイズが変えられる必要が出てきます。comelangはどちらの書き方もできるので、場合に応じてサイズが変えられるヒープにデータを確保するのか、それともサイが変えられないが、より高速な静的領域にデータを確保するのか、選んでください。
...
としても同じです。文字列処理のライブラリとしてcomelang-str.hもありますが、こちらは標準CライブラリだけでなくpcreというUNIXのライブラリに依存しているためUNIXでしか動きせん。comelang-str.hは大体Rubyと同じように動くため、ここでは解説を省かせてもらいます。詳しくはソースを見てください。ここではcomelang.hに定義された、基本的な文字列処理のライブラリを解説させていただきます。これは標準Cしか依存していないため、マイコンなどでも動きます。
...
その場合は`int fun() version 2`として上書きします。`inherit()`は元の関数を呼び出すもので、単`fun()`と呼ばれているのと同じです。
...
そのように既存のライブラリやシステムコールを再定義したい場合に便利だ思います。
...
となります。実はこの複数の戻り値を返せないというのはC言語の最大の弱点で、Cで書かれた標準Cライラリやシステムコールなどは多くはint型を返しますが
...
3回`printf("%d\n", it);`が実行されます。メソッドブロックは引数は宣言が必要ありません。it, it2, it3...に代入されます。timesの場合のitは実行し回数です。メソッドブロックの中ではcontinueやbreakなどが使えません。
...
関数の中でも貼り付けられます。文字列しか引数に渡せないため、数値を渡しいときは一旦変数に代入して変数を渡してください。
...
!!による例外ハンドリング：!!を使って関数の呼び出しをうと、例外がスローされていない場合は正常値を受け取り、例外が発生した場合はエラーメッセージが出力されます。
...
もしelseブロックを省くと、戻り値はfun(1)の戻りとなります。elifにマッチした場合だけ、この場合だと`s"null"`が代入されます。
...
`==0`の場合はdie実行されエラーメッセージを表示してプログラムが終了します。
...
とりあえず、完成しました。今回は満足です。versionも10.0.0としておきたいと思います。
何年ぐらいやったんでしょうね。3年くらいですかね。まあ、楽しかったです。
今回は悔いはありません。やり切った感じがあります。
プログラミングは家事みたいなもので、永遠に終わりがありません。あなたが完成したと思った時が終わりです。
あとはwebサーバーとデータベースサーバーの続きをしたり、vinというviクローンの改良などをしたいかなと思います。
チャネルを使って遊んでみたいと思います。あとマイコンですかね。m5stackもraspi picoも持っていますけど。
OS作りとかの夢はありましたんで、トライしますかねぇ、、、5年は飛んでいきそうorz
まあ、今回の自信作です。clover2は拡張ライブラリを作ってもらえないと意味なかったですが、comelangはC言語のライブラリが使えるため、その心配もないです。
それが大きいですね。結局メーカーが指定する言語じゃないと、そのメーカーのライブラリが使えないっていうのが辛いですね。フリーソフトの場合。
最近はAIがあるので、新しい分野に挑戦するのが楽になりましたね。OS作りでも挑戦しますか。まだまだプログラミングが楽しめそうです。
皆さんもプログラミングを社会生活を犠牲にしない程度で楽しんでくださいくれぐれもプログラミング一筋とかやめてください。プログラミングには終わりがないので。
僕も終わらない物語を続けます。では。enjoy programing
