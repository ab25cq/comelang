...
最初のintというのは64bitの数値型で、まあ、普通に一般的な数字を扱う数値型と覚えればいいでしょう。これは関数の戻り値の型を定義しています。この場合main関数という関数の実行結果を数値型としています。main関数はint型を返さないといけないとされていて、main関数の戻り値がシェルの`$?`という環境変数に入れられるコマンドのステータスコードとされています。
...
comelang new XでXというプロジェクトを作っています。その中でa.cというソースファイルとb.cというソースファイルを作っています。
a.cはb.cで定義されたfunという関数を使っています。本来であれば、b.cのヘッダーファイルを作成しないといけませんが、comelangは自動的にヘッダーファイルを作ってくれます。
comelang compileすると.cという拡張子があるファイルからヘッダーファイルcommon.hを自動的に作成して、a.cとb.cで自動的に\#includeします。
...
comelangはcppがあるため使い物になりません。使わないでください。
...
3回`printf("%d\n", it);`が実行されます。メソッドブロックは引数は宣言が必要ありません。it, it2, it3...に代入されます。timesの場合のitは実行した回数です。メソッドブロックの中ではcontinueやbreakなどが使えません。
...
皆さんもプログラミングを社会生活を犠牲にしない程度で楽しんでください。くれぐれもプログラミング一筋とかやめてください。プログラミングには終わりがないので。
