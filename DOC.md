...
でsChildA, sChildB両方にshowで表示されます。
...
string型の文字列には`\{}`で式を埋め込めます。`$`で変数を展開できます。
...
複数行の文字列です。最後の行の"""は行頭は無視されます。`\{}`で変数を展開できます。`$`は展開しません。
...
継承は上記のようにします。多重継承はありません。sData2にshowメソッドも呼び出せます。
...
文字列しか引数に渡せないため、数値を渡したいときは一旦変数に代入して変数を渡してください。
