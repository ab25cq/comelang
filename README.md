...
With version 10.0.0, we were suported with struct initializer, array initializer, union initializer. We can call comelang as C compiler.
...
This language is infinity self-host.
...
xsprintf is a function that returns the memory of a format string. Returns a string allocated by the GC. === internally is operator_equal ~
== will not be true unless the pointer points to the same address, but === will return true if they have the same value.
...
Since sort_default () internally calls compare, you need to implement compare.
...
This way sData can also be sort_default
...
foreach accesses all elements with an iterator
