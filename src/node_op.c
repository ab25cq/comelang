        ...
        *found = TRUE;
        ...
        if(operator_fun->mGenericsFunction) {
            ...
            if(!move_) {
                ...
                obj = LLVMBuildCall(gBuilder, llvm_fun, llvm_params, num_params, "funXXX");
            }
            ...
            if(!move_) {
                ...
                obj = LLVMBuildCall(gBuilder, llvm_fun, llvm_params, num_params, "funYYY");
            }
...
BOOL compile_add(unsigned int node, sCompileInfo* info)
{
    ...
    if(!call_operator_function(fun_name, left_type, num_params, params, &found, move_, info)) {
        return FALSE;
    }
    ...
    if(!found) {
        if(left_type->mArrayDimentionNum == 1 && is_number_type(right_type))
        {
            ...
            llvm_value.value = LLVMBuildCast(gBuilder, LLVMIntToPtr, llvm_value.value, llvm_var_type, "intToPtrA");
...
BOOL compile_complement(unsigned int node, sCompileInfo* info)
{
    ...
    if(!call_operator_function(fun_name, left_node_type, num_params, params, &found, TRUE, info)) {
        return FALSE;
    }
