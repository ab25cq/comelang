...
//printf("%s %d %d\n", info->sname, info->sline, gNodes[node].mNodeType);
    ...
    switch(gNodes[node].mNodeType) {
        ...
        case kNodeTypeInlineCallerSName:
            if(!compile_inline_caller_sname(node, info)) {
                return FALSE;
            }
            break;
            
        case kNodeTypeInlineCallerSLine:
            if(!compile_inline_caller_sline(node, info)) {
                return FALSE;
            }
            break;
