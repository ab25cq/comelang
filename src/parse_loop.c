    ...
    while(1) {
        ...
        if(strcmp(buf, "else") == 0) {
            if(parse_cmp(info->p, "if") == 0) {
                ...
                BOOL result_type_is_void = TRUE;
                if(!parse_block_easy(ALLOC &elif_node_blocks[elif_num], FALSE, result_type_is_void, info))
                {
                    return FALSE;
                }
                ...
                BOOL result_type_is_void = TRUE;
                if(!parse_block_easy(ALLOC &else_node_block, FALSE, result_type_is_void, info))
                {
                    return FALSE;
                }
                break;
...
BOOL parse_while(unsigned int* node, sParserInfo* info)
{
    ...
    BOOL result_type_is_void = TRUE;
    if(!parse_block_easy(ALLOC &while_node_block, FALSE, result_type_is_void, info))
    {
        return FALSE;
    };
...
BOOL parse_for(unsigned int* node, sParserInfo* info)
{
    ...
    sNodeBlock* for_node_block = ALLOC sNodeBlock_alloc();
    BOOL result_type_is_void = TRUE;
    if(!parse_block(for_node_block, FALSE, single_expression, result_type_is_void, FALSE, info)) 
    {
        return FALSE;
    }
...
BOOL parse_select(unsigned int* node, sParserInfo* info)
{
    ...
    while(TRUE) {
        ...
        BOOL result_type_is_void = TRUE;
        if(!parse_block_easy(ALLOC &pipe_blocks[num_pipes], FALSE, result_type_is_void, info))
        {
            return FALSE;
        }
...
BOOL parse_pselect(unsigned int* node, sParserInfo* info)
{
    ...
    while(TRUE) {
        ...
        if(strcmp(buf, "default") == 0) {
            BOOL result_type_is_void = TRUE;
            if(!parse_block_easy(ALLOC &default_block, FALSE, result_type_is_void, info))
            {
                return FALSE;
            }
        }
