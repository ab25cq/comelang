...
BOOL call_inline_function(sFunction* fun, sNodeType* generics_type, int num_method_generics_types, sNodeType** method_generics_types, LLVMValueRef* llvm_params, sCompileInfo* info)
{
    ...
    int inline_caller_sline = info->inline_caller_sline;
    char inline_caller_sname[VAR_NAME_MAX];
    xstrncpy(inline_caller_sname, info->inline_caller_sname, VAR_NAME_MAX);
    
    xstrncpy(info->inline_caller_sname, info->sname, VAR_NAME_MAX);
    info->inline_caller_sline = info->sline;
    ...
    for(i=0; i<num_params; i++) {
        BOOL readonly = FALSE;
        if(!add_variable_to_table(info2.lv_table, param_names[i], fun->mParamTypes[i], readonly, NULL, -1, FALSE, param_types[i]->mConstant, FALSE))
        {
    ...
    info->inline_caller_sline = inline_caller_sline;
    xstrncpy(info->inline_caller_sname, inline_caller_sname, VAR_NAME_MAX);
...
//        params[i] = gNodes[node].uValue.sFunctionCall.mParams[i];
    ...
    for(i=0; i<fun->mNumParams; i++) {
        ...
        if(fun_param_type->mHeap && param.type->mHeap && !param.type->mDummyHeap) {
            ...
            if(var_) {
            /*
                if(fun_param_type->mRefference) {
                    param.address = var_->mLLVMValue;
                    LLVMValueRef obj = LLVMBuildLoad(gBuilder, var_->mLLVMValue, "std_move_obj");
                    param.value = clone_object(param_type, obj, info);
                    remove_object_from_right_values(param.value, info);
    ...
    if(fun->mGenericsFunction) {
        ...
        if(type_identify_with_class_name(result_type, "void") && result_type->mPointerNum == 0) {
            ...
            LVALUE llvm_value;
            llvm_value.value = LLVMBuildCall(gBuilder, llvm_fun, llvm_params, num_params, "fun_result2");
...
BOOL compile_lambda_call(unsigned int node, sCompileInfo* info)
{
    ...
    if(type_identify_with_class_name(lambda_type->mResultType, "void") && lambda_type->mResultType->mPointerNum == 0)
    {
        ...
        LVALUE llvm_value;
        llvm_value.value = LLVMBuildCall(gBuilder, lambda_value.value, llvm_params, num_params, "fun_result3");
        llvm_value.type = result_type;
        llvm_value.address = NULL;
        llvm_value.var = NULL;
