...
BOOL parse_param(sParserParam* param, sParserInfo* info)
{
    ...
    if(!parse_type(&param->mType, info, param->mName, FALSE, FALSE, NULL, FALSE, FALSE)) {
        return FALSE;
    }
...
BOOL parse_method_generics_function(unsigned int* node, char* struct_name, sParserInfo* info)
{
    ...
    if(!parse_type(&result_type, info, NULL, FALSE, FALSE, NULL, FALSE, TRUE)) 
    {
        return FALSE;
    }
    ...
    if(*info->p == ';') {
        ...
        *node = sNodeTree_create_external_function(fun_name, asm_fname, params, num_params, var_arg, result_type, struct_name, operator_fun, version, immutable_, flag_asm_fun_name, info);
        ...
        for(i=0; i<num_params; i++) {
            ...
            BOOL readonly = FALSE;
            if(!add_variable_to_table(info->lv_table, param->mName, param->mType, readonly, NULL, -1, FALSE, param->mType->mConstant, FALSE))
            {
                return FALSE;
            }
        }
        ...
        if(gNCHeader) {
            ...
            if(!parse_block(node_block, FALSE, FALSE, result_type_is_void, FALSE, info)) {
                sNodeBlock_free(node_block);
                return FALSE;
            }
...
BOOL parse_class(unsigned int* node, sParserInfo* info)
{
    ...
    if(!parse_block(node_block, FALSE, FALSE, result_type_is_void, return_self, info)) {
        sNodeBlock_free(node_block);
        return FALSE;
    }
...
BOOL parse_funcation_call_params(int* num_params, unsigned int* params, sParserInfo* info)
{
    ...
    if(is_type_name(buf, info)) {
        ...
        parse_type(&result_type, info, NULL, FALSE, FALSE, NULL, FALSE, TRUE);
    }
    else {
        info->p = p;
        info->sline = sline;
...
BOOL parse_function_pointer_result_function(unsigned int* node, BOOL* array_pointer_result_function_type, sParserInfo* info)
{
    ...
    if(!parse_type(&node_type, info, NULL, FALSE, FALSE, NULL, TRUE, TRUE))
    {
        return FALSE;
    }
