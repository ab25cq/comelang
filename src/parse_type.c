...
BOOL parse_type(sNodeType** result_type, sParserInfo* info, char* func_pointer_name, BOOL definition_typedef, BOOL definition_struct, BOOL* function_pointer_result_function, BOOL definition_function_pointer_result_function, BOOL tuple_type_name)
{
        ...
        else if(strcmp(type_name, "struct") == 0 || strcmp(type_name, "protocol") == 0)
        {
            ...
            if(xisalpha(*info->p) || *info->p == '_') {
                ...
                if(*info->p == '{') {
                    ...
                    if(!parse_struct(&node, struct_name, VAR_NAME_MAX, anonymous, NULL, definition_struct, protocol_, static_, info)) {
                        return FALSE;
                    }
...
*/
                ...
                if(*info->p == '(') {
                    ...
                    if(*info->p == ')') {
                        ...
                        while(1) {
                            ...
                            if(!parse_type(&node_type, info, NULL,  FALSE, FALSE, NULL, FALSE, FALSE)) {
                                return FALSE;
                            }
                        ...
                        if(*info->p == '(') {
                            ...
                            if(*info->p == ')') {
                                ...
                                while(1) {
                                    ...
                                    if(!parse_type(&node_type, info, func_pointer_name2,  FALSE, FALSE, NULL, FALSE, FALSE)) {
                                        return FALSE;
                                    }
    ...
    if(*info->p == '<' && *(info->p+1) != '<' && *(info->p+1) != '=') 
    {
        ...
        while(1) {
            ...
            if(!parse_type(&(*result_type)->mGenericsTypes[generics_num], info, func_pointer_name, FALSE, FALSE, NULL, FALSE, FALSE)) {
                return FALSE;
            }
...
*/
        ...
        if(*info->p == '(') {
            ...
            if(*info->p == ')') {
                ...
                while(1) {
                    sNodeType* node_type = NULL;
                    char buf[VAR_NAME_MAX];
                    if(!parse_type(&node_type, info, buf, FALSE, FALSE, NULL, FALSE, FALSE)) {
                        return FALSE;
                    }
    ...
    if(tuple_type_name && info->comma_op) {
        ...
        while(*info->p == ',') {
            ...
            if(!parse_type(&node_type3, info, NULL, FALSE, FALSE, NULL, FALSE, FALSE)) {
                return FALSE;
            }
