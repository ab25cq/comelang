...
static BOOL expression_node_variable_method_block_parent(unsigned int* node, BOOL enable_assginment, char* buf, sParserInfo* info)
{
    if(enable_assginment && *info->p == '=' && *(info->p+1) != '=') {
        ...
        if(right_node == 0) {
            ...
            *node = sNodeTree_create_store_value_to_address(*node, right_node, TRUE, NULL, info);
        }
    }
    else if(*info->p == '+' && *(info->p+1) == '+')
    {
        ...
        *node = sNodeTree_create_add(left_node, right_node, 0, TRUE, info);
        ...
        unsigned int left_node2 = sNodeTree_create_store_value_to_address(node3, *node, TRUE, NULL, info);
    ...
    else if(*info->p == '*' && *(info->p+1) == '=')
    {
        ...
        *node = sNodeTree_create_mult(left_node, right_node, 0, info);
        ...
        unsigned int node2 = sNodeTree_create_load_variable("parent", info);
        unsigned int node3 = sNodeTree_create_load_field(buf, node2, info);
        *node = sNodeTree_create_store_value_to_address(node3, *node, TRUE, NULL, info);
    }
    else if(*info->p == '/' && *(info->p+1) == '=')
    {
        info->p+=2;
...
BOOL expression_node(unsigned int* node, BOOL enable_assginment, sParserInfo* info)
{
    ...
    if(*info->p == '(') {
        ...
        if(!parse_block_easy(ALLOC &node_block, FALSE, result_type_is_void, info))
        {
            return FALSE;
        };
    ...
    else if(*info->p == '}' && strcmp(info->impl_struct_name, "") != 0)
    {
        ...
        *node = sNodeTree_create_null(info);
        ...
        if(*info->p == '=' && *(info->p+1) != '=') {
            ...
            *node = sNodeTree_create_store_value_to_address(*node, node2, FALSE, cast_pointer_type, info);
        }
        else if(*info->p == '+' && *(info->p+1) == '+')
        {
            info->p+=2;
    ...
    /// chararacter ///
        ...
        if(strcmp(buf, "struct") == 0 || strcmp(buf, "protocol") == 0) {
            BOOL protocol_ = strcmp(buf, "protocol") == 0;
            ...
            if(*info->p == '{') {
                ...
                BOOL static_ = TRUE;
                if(!parse_struct(node, struct_name, VAR_NAME_MAX, anonymous, &terminated, definition_struct, protocol_, static_, info)) {
                    return FALSE;
                }
        ...
        else if((strcmp(buf, "struct") == 0 || strcmp(buf, "protocol") == 0) && *info->p != '{' && define_struct) {
            BOOL protocol_ = strcmp(buf, "protocol") == 0;
        ...
        else if(strcmp(buf, "__inline_caller_sline__") == 0) 
        {
            *node = sNodeTree_create_inline_caller_sline(info);
        }
        else if(strcmp(buf, "__inline_caller_sname__") == 0) 
        {
            *node = sNodeTree_create_inline_caller_sname(info);
        }
        else if(strcmp(buf, "__alignof__") == 0 || strcmp(buf, "_Alignof") == 0) 
        {
            ...
            *node = sNodeTree_create_continue_expression(info);
            ...
            if(!parse_type(&result_type, info, name, FALSE, FALSE, &function_pointer_result_function, FALSE, TRUE))
            {
                return FALSE;
            }
            ...
            if(function_pointer_result_function) {
                ...
                if(array_result_function) {
                    sNodeType* result_type = NULL;
                    if(!parse_type(&result_type, info, NULL, FALSE, FALSE, NULL, TRUE, TRUE))
                    {
                        return FALSE;
                    }
            ...
            if(function_pointer_result_function) {
                ...
                if(result_type->mArrayPointer) {
                    if(*info->p == '=') {
                        ...
                        if(!add_variable_to_table(info->lv_table, name, result_type, FALSE, NULL, -1, global, FALSE, FALSE))
                        {
