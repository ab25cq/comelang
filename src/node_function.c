...
*/
            ...
            if(num_params == fun->mNumParams) {
                for(i=0; i<num_params; i++) {
                    ...
                    {
                        compile_err_msg(info, "Different extern function and definition of the function");
                ...
                compile_err_msg(info, "Different extern function param number and definition of the function param number");
                return TRUE;
            }
        }
    }
    ...
    if(!add_function_to_table(fun_name, num_params, param_names2, param_types, param_default_values2, param_labels, result_type, llvm_fun, block_text, generics_function, var_arg, 0, NULL, 0, NULL, FALSE, asm_fun_name, TRUE, node_block->mSource.mBuf, info->sname, info->sline, immutable_, flag_asm_fun_name)) {
    ...
    if(strcmp(fun_name, "main") == 0 && gNCGC) {
        ...
        if(llvm_fun != NULL) {
            int num_params = 0;
            LLVMBuildCall(gBuilder, llvm_fun, NULL, num_params, "");
        }
    }
    ...
    if(type_identify_with_class_name(result_type, "void") && result_type->mPointerNum == 0) {
        ...
        if(!info->last_expression_is_return) {
            ...
            if(((result_type->mClass->mFlags & CLASS_FLAGS_STRUCT) || (result_type->mClass->mFlags & CLASS_FLAGS_UNION)) && result_type->mPointerNum == 0) {
                ...
                llvm_value.value = LLVMBuildAlloca(gBuilder, llvm_type, "result_object");
                llvm_value.value = LLVMBuildLoad(gBuilder, llvm_value.value, "result_object");
...
BOOL compile_inline_function(unsigned int node, sCompileInfo* info)
{
    ...
    BOOL generics_function = FALSE;
    BOOL flag_asm_fun_name = FALSE;
    if(!add_function_to_table(fun_name, num_params, param_names2, param_types, param_default_values2, param_labels, result_type, llvm_fun, block_text, generics_function, var_arg, 0, NULL, 0, NULL, FALSE, NULL, TRUE, block_text, info->sname, info->sline, immutable_, flag_asm_fun_name)) {
        fprintf(stderr, "overflow function table\n");
        return FALSE;
    }
...
unsigned int sNodeTree_create_method_block(MANAGED char* block, char* block_text_sname, int block_text_sline, sNodeType* result_type, char* current_stack_frame_type_name, sParserInfo* info)
{
    ...
    xstrncpy(gNodes[node].uValue.sMethodBlock.mCurrentStackFrameTypeName, current_stack_frame_type_name, VAR_NAME_MAX);
...
//    create_current_stack_frame_struct(type_name, lv_table);
    ...
    for(i=0; i<num_params; i++) {
        ...
        BOOL readonly = TRUE;
        if(!add_variable_to_table(pinfo.lv_table, param->mName, param->mType, readonly, NULL, -1, FALSE, param->mType->mConstant, FALSE))
        {
            return FALSE;
        }
    }
    ...
    if(!parse_block(node_block, FALSE, single_expression, result_type_is_void, FALSE, &pinfo)) {
...
BOOL compile_external_function(unsigned int node, sCompileInfo* info)
{
    ...
    if(llvm_fun) {
        if(version > 0) {
            ...
            BOOL generics_function = FALSE;
            if(!add_function_to_table(new_fun_name, num_params, param_names2, param_types, param_default_values2, param_labels, result_type, llvm_fun, block_text, generics_function, var_arg, 0, NULL, 0, NULL, TRUE, asm_fun_name, TRUE, NULL, info->sname, info->sline, immutable_, flag_asm_fun_name)) {
                fprintf(stderr, "overflow function table\n");
                return FALSE;
            }
        ...
        if(version > 0) {
            ...
            BOOL generics_function = FALSE;
            if(!add_function_to_table(fun_name, num_params, param_names2, param_types, param_default_values2, param_labels, result_type, llvm_fun, block_text, generics_function, var_arg, 0, NULL, 0, NULL, TRUE, asm_fun_name, TRUE, NULL, info->sname, info->sline, immutable_, flag_asm_fun_name)) {
                fprintf(stderr, "overflow function table\n");
                return FALSE;
            }
        }
    ...
    *llvm_fun = LLVMGetNamedFunction(gModule, real_fun_name);
    ...
    if(*llvm_fun == NULL) {
        ...
        for(i=0; i<num_params; i++) {
            ...
            BOOL readonly = FALSE;
            if(!add_variable_to_table(info2.lv_table, param.mName, param.mType, readonly, NULL, -1, FALSE, param.mType->mConstant, FALSE))
            {
        ...
        if(!parse_block(node_block, FALSE, FALSE, result_type_is_void, FALSE, &info2)) {
            sNodeBlock_free(node_block);
            return FALSE;
        }
