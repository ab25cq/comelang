                ...
                /// access fields ///
                    ...
                    if(enable_assginment && *info->p == '=' && *(info->p +1) != '=' && *(info->p+1) != '>') {
                        ...
                        if(*info->p == '+' && *(info->p+1) == '+')
                        {
                            ...
                            *node = sNodeTree_create_add(field_node, right_node, TRUE, 0, info);
                            *node = sNodeTree_create_store_field(var_name, obj_node, *node, info);
                            
                            *node = sNodeTree_create_sub(*node, right_node, 0, FALSE, info);
                        }
                        else if(*info->p == '-' && *(info->p+1) == '-')
                        {
                            ...
                            *node = sNodeTree_create_sub(field_node, right_node, 0, TRUE, info);
                            *node = sNodeTree_create_store_field(var_name, obj_node, *node, info);
                            *node = sNodeTree_create_add(*node, right_node, 0, FALSE, info);
                        }
                        else if(enable_assginment && *info->p == '+' && *(info->p+1) == '=')
                        {
                            ...
                            *node = sNodeTree_create_add(field_node, right_node, 0, TRUE, info);
                            *node = sNodeTree_create_store_field(var_name, obj_node, *node, info);
                        }
                        else if(enable_assginment && *info->p == '-' && *(info->p+1) == '=')
                        {
...
BOOL parse_var(unsigned int* node, sParserInfo* info, BOOL readonly)
{
    ...
    for(i=0; i<num_vars; i++) {
        ...
        if(!add_variable_to_table(info->lv_table, buf[i], node_type, readonly, NULL, -1, info->mBlockLevel == 0, FALSE, FALSE))
        {
