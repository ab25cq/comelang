...
sVarTable* clone_var_table(sVarTable* lv_table)
{
    ...
    while(1) {
        if(p->mName[0] != 0) {
            if(!add_variable_to_table(result, p->mName, p->mType, p->mReadOnly, p->mLLVMValue, p->mIndex, p->mGlobal, p->mConstant, p->mAllocaValue))
            {
...
BOOL add_variable_to_table(sVarTable* table, char* name, sNodeType* type_, BOOL readonly, void* llvm_value, int index, BOOL global, BOOL constant, BOOL alloca_value)
{
    int hash_value;
    sVar* p;
                ...
                if(node_type->mHeap)
                {
                    if(p->mLLVMValue)
                    {
                        if(p->mConstant) {
                            ...
                            LLVMValueRef obj = LLVMBuildLoad(gBuilder, p->mLLVMValue, "vtable_obj");
                            free_object(p->mType, obj, FALSE, info);
                            remove_object_from_right_values(obj, info);
                        }
