...
BOOL compile_come_function_call(unsigned int node, sCompileInfo* info)
{
    ...
    BOOL readonly = FALSE;
    if(!add_variable_to_table(info->pinfo->lv_table, thread_var_name, var_type, readonly, NULL, -1, FALSE, FALSE, FALSE))
    {
        return FALSE;
    }
    ...
    if(!add_variable_to_table(info->pinfo->lv_table, thread_arg_var_name, thread_arg_struct_type, FALSE, NULL, -1, FALSE, FALSE, FALSE))
    {
        return FALSE;
    }
    ...
    if(!add_variable_to_table(pinfo.lv_table, thread_fun_params[0].mName, thread_fun_params[0].mType, readonly, NULL, -1, FALSE, FALSE, FALSE))
    {
        return FALSE;
    }
    ...
    if(!parse_block_easy(&thread_fun_block, FALSE, result_type_is_void, &pinfo))
    {
        return FALSE;
    }
