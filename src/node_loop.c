...
//        LLVMBuildBr(gBuilder, cond_else_block);
    ...
    if(expression_node == 0) {
        ...
        if(auto_cast_posibility(left_type, rvalue.type) || left_type->mPointerNum > 0 && is_number_type(right_type))
        {
...
BOOL compile_select(unsigned int node, sCompileInfo* info)
{
    ...
    if(!parse_block_easy(&node_block, FALSE, result_type_is_void, &pinfo)) {
        return FALSE;
    }
