...
LLVMValueRef defineStringLiteral( const char *sourceString, size_t size ) 
{
    ...
    LLVMValueRef str = LLVMAddGlobal(gModule, strType, "");
    LLVMSetInitializer(str, LLVMConstString( sourceString, size, FALSE ));
...
BOOL compile_c_string_value(unsigned int node, sCompileInfo* info)
{
    ...
    if(info->pinfo->mBlockLevel == 0) {
        ...
        LLVMValueRef value = defineStringLiteral(buf, strlen(buf));
        ...
        LLVMValueRef value = LLVMBuildGlobalString(gBuilder, buf, buf);
...
BOOL compile_regex_value(unsigned int node, sCompileInfo* info)
{
    ...
    LLVMValueRef result = LLVMBuildCall(gBuilder, llvm_fun, llvm_params, num_params, "fun_resultOOO");
