...
BOOL get_list(unsigned int* node, sParserInfo* info)
{
    ...
    if(!add_variable_to_table(info->lv_table, var_name, NULL, FALSE, NULL, -1, info->mBlockLevel == 0, FALSE, FALSE))
    {
    ...
    nodes[num_nodes++] = sNodeTree_create_load_variable(var_name, info);
...
BOOL parse_struct(unsigned int* node, char* struct_name, int size_struct_name, BOOL anonymous, BOOL* terminated, BOOL definition_struct, BOOL protocol_, BOOL static_, sParserInfo* info) 
{
...
*/
    ...
    if(gNCHeader) {
        ...
        while(TRUE) {
            ...
            char buf[VAR_NAME_MAX];
            if(!parse_type(&field, info, buf, FALSE, TRUE, NULL, FALSE, TRUE)) {
                info->in_struct = in_struct;
                return FALSE;
            }
        ...
        while(TRUE) {
            ...
            if(terminated == NULL) {
                ...
                if(var_ == NULL) {
                    ...
                    check_already_added_variable(info->lv_table, var_name, info);
                    BOOL readonly = TRUE;
                    if(!add_variable_to_table(info->lv_table, var_name, result_type, readonly, NULL, -1, info->mBlockLevel == 0, result_type->mConstant, FALSE))
                    {
                        fprintf(stderr, "overflow variable table\n");
                        exit(2);
                    }
...
BOOL parse_typedef(unsigned int* node, BOOL static_, sParserInfo* info)
{
    ...
    if(!parse_type(&node_type, info, buf, TRUE, FALSE, NULL, FALSE, FALSE)) {
        return FALSE;
    }
...
BOOL parse_lambda(unsigned int* node, sNodeType* result_type, sParserInfo* info)
{
    ...
    if(!parse_block(node_block, FALSE, single_expression, result_type_is_void, FALSE, info)) {
...
//        }
    ...
    if(!add_variable_to_table(info->lv_table, name, result_type, readonly, NULL, -1, info->mBlockLevel == 0, result_type->mConstant, FALSE))
    {
        fprintf(stderr, "overflow variable table\n");
        exit(2);
    }
