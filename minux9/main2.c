...
// ------------------------------------------------------------
//   (DEBUG )  legacy virtioblk driver + fs.img reader
// ------------------------------------------------------------
//  * RISCV QEMU "virt"  virtiommio (version=1, PFN ) 
//  * slot07  deviceid==2 MMIO base 
//  * 4 KiB page : desc[07] | avail | () | used(2KiB)
//  * WRITE 
//  * virtio_debug_dump() /flags/PFN/used.idx 
//  * read_entire_fsimg(): 256  (128 KiB) 
// ------------------------------------------------------------
...
#define NUM_DESC      8          // fixed
#define FSIMG_SECTORS 256        // 128 KiB sample
...
struct virtq_avail { uint16_t flags, idx; uint16_t ring[NUM_DESC]; } __attribute__((packed,aligned(2)));
struct virtq_used_elem{ uint32_t id,len; } __attribute__((packed));
struct virtq_used  { uint16_t flags, idx; struct virtq_used_elem ring[NUM_DESC]; } __attribute__((packed,aligned(4)));
...
//-------------------------------------------------- virtqueue page (manual layout)
#define VQ_PHYS 0x80020000UL                     // identitymapped phys addr
static uint8_t vq_page[4096] __attribute__((aligned(4096)));

static struct virtq_desc  *desc  = (void*)vq_page;                    // 0127
static struct virtq_avail *avail = (void*)(vq_page + 16*NUM_DESC);    // 128151
static struct virtq_used  *used  = (void*)(vq_page + 2048);           // 2 KiB boundary

//-------------------------------------------------- globals
static uint64_t virtio_base = 0;       // detected mmio base
...
//-------------------------------------------------- debug helper
static void virtio_debug_dump(const char* tag)
{
    printf("[%s] PFN_drv=%lx PFN_dev=%x used.idx=%u flags0=%x flags1=%x flags2=%x status=%x\n",
        tag,
        VQ_PHYS>>12,
        mr32(virtio_base, REG_QUEUE_PFN),
        used->idx,
        desc[0].flags, desc[1].flags, desc[2].flags,
        mr32(virtio_base, REG_STATUS));
}
...
void virtio_disk_init(void)
{
    ...
    for(int s=0;s<VIRTIO_MAX_SLOTS;++s){
        ...
        if(mr32(base,REG_MAGIC_VALUE)!=0x74726976) continue;
        if(mr32(base,REG_DEVICE_ID)==2){ virtio_base=base; break; }
    }
    if(!virtio_base){ printf("virtioblk not found\n"); while(1);}    
    if(mr32(virtio_base,REG_VERSION)!=1){ printf("not legacy mode\n"); while(1);}  

    mw32(virtio_base, REG_STATUS, 0);
    mw32(virtio_base, REG_STATUS, STATUS_ACK | STATUS_DRIVER);

    mw32(virtio_base, REG_GUEST_PAGE_SIZE, 4096);

    mw32(virtio_base, REG_QUEUE_SEL, 0);
    uint32_t qsz = mr32(virtio_base, REG_QUEUE_NUM);
    if(qsz < NUM_DESC){ printf("queue too small %u\n",qsz); while(1);}  

    mw32(virtio_base, REG_QUEUE_ALIGN, 4096);
    mw32(virtio_base, REG_QUEUE_PFN, (uint32_t)(VQ_PHYS >> 12));

    memset(vq_page, 0, 4096);

    mw32(virtio_base, REG_STATUS, STATUS_ACK | STATUS_DRIVER | STATUS_DRIVER_OK);
    virtio_debug_dump("init");
}

//-------------------------------------------------- read sector (blocking)
void virtio_disk_read_sector(uint64_t sector, void *dst)
{
    struct virtio_blk_req req = { .type = VIRTIO_BLK_T_IN, .sector = sector };
    status_byte = 0xFF;

    desc[0] = (struct virtq_desc){ .addr=(uint64_t)&req, .len=sizeof(req),
                                   .flags=VIRTQ_DESC_F_NEXT, .next=1 };

    desc[1] = (struct virtq_desc){ .addr=(uint64_t)dma_buf, .len=SECTOR_SIZE,
                                   .flags=VIRTQ_DESC_F_NEXT|VIRTQ_DESC_F_WRITE, .next=2 };

    desc[2] = (struct virtq_desc){ .addr=(uint64_t)&status_byte, .len=1,
                                   .flags=VIRTQ_DESC_F_WRITE };

    avail->ring[avail_idx % NUM_DESC] = 0;
    __sync_synchronize();
    avail->idx = ++avail_idx;

    virtio_debug_dump("before notify");
    mw32(virtio_base, REG_QUEUE_NOTIFY, 0);

    while(status_byte == 0xFF) __sync_synchronize();

    virtio_debug_dump("after DMA");
...
{
    uart_init();
    printf("--- virtioblk legacy debug test ---\n");
    virtio_disk_init();
    read_entire_fsimg();
    printf("done\n");
    while(1);
...
/*
// ------------------------------------------------------------
//  virtio-blk minimal driver + fs.img reader (modern virtiommio)
//  *  block(deviceid==2) 
//  * QueueReady + FEATURES_OK + DRIVER_OK 
//  *  + fs.img(256sect) 
// ------------------------------------------------------------
// 20250605  (public domain / no warranty)
...
static uint64_t virtio_base=0;   //  mmio base 
...
void virtio_disk_init(void)
{
    ...
    for(int s=0;s<VIRTIO_MAX_SLOTS;s++){
        ...
        if(mmio_read32(base,REG_DEVICE_ID)==2){ virtio_base=base; break; }
    }
    if(!virtio_base){ printf("virtioblk not found\n"); while(1); }

    // --- device reset + basic status ---
    mmio_write32(virtio_base,REG_STATUS,0);
    mmio_write32(virtio_base,REG_STATUS,STATUS_ACK|STATUS_DRIVER);

    // --- queue 0 select ---
    mmio_write32(virtio_base,REG_QUEUE_SEL,0);
    uint32_t max=mmio_read32(virtio_base,REG_QUEUE_NUM_MAX);
    if(max<NUM_DESC){ printf("queue too small\n"); while(1);}    
    mmio_write32(virtio_base,REG_QUEUE_NUM,NUM_DESC);

    // features (0)
    mmio_write32(virtio_base,REG_DRIVER_FEATURES,0);
    mmio_write32(virtio_base,REG_STATUS,STATUS_ACK|STATUS_DRIVER|STATUS_FEATURES_OK);
    if(!(mmio_read32(virtio_base,REG_STATUS)&STATUS_FEATURES_OK)){
        printf("FEATURES_OK rejected\n"); while(1);
    }

    // queue 
    uint64_t pa_desc =(uint64_t)(uintptr_t)desc;
    uint64_t pa_av   =(uint64_t)(uintptr_t)&avail;
    uint64_t pa_used =(uint64_t)(uintptr_t)&used;

    mmio_write32(virtio_base,REG_QUEUE_DESC_LOW, (uint32_t)pa_desc);
    mmio_write32(virtio_base,REG_QUEUE_DESC_HIGH,(uint32_t)(pa_desc>>32));
    mmio_write32(virtio_base,REG_QUEUE_AVAIL_LOW,(uint32_t)pa_av);
    mmio_write32(virtio_base,REG_QUEUE_AVAIL_HIGH,(uint32_t)(pa_av>>32));
    mmio_write32(virtio_base,REG_QUEUE_USED_LOW, (uint32_t)pa_used);
    mmio_write32(virtio_base,REG_QUEUE_USED_HIGH,(uint32_t)(pa_used>>32));

    // queue ready & DRIVER_OK
    mmio_write32(virtio_base,REG_QUEUE_READY,1);
    mmio_write32(virtio_base,REG_STATUS,STATUS_ACK|STATUS_DRIVER|STATUS_FEATURES_OK|STATUS_DRIVER_OK);
}

//------------------------------------------------------------
// 1 sector read (blocking)
//------------------------------------------------------------
void virtio_disk_read_sector(uint64_t sector, void *dst)
{
    struct virtio_blk_req req={.type=VIRTIO_BLK_T_IN,.sector=sector};
    ...
    desc[0]=(struct virtq_desc){.addr=(uint64_t)(uintptr_t)&req, .len=sizeof(req), .flags=VIRTQ_DESC_F_NEXT, .next=1};
    desc[1]=(struct virtq_desc){.addr=(uint64_t)(uintptr_t)dma_buf, .len=SECTOR_SIZE, .flags=VIRTQ_DESC_F_NEXT, .next=2};
    desc[2]=(struct virtq_desc){.addr=(uint64_t)(uintptr_t)&status_byte, .len=1, .flags=VIRTQ_DESC_F_WRITE};

    avail.ring[avail_idx%NUM_DESC]=0;
    __sync_synchronize();
    avail.idx=++avail_idx;

    mmio_write32(virtio_base,REG_QUEUE_NOTIFY,0);
...
void read_entire_fsimg(void)
{
    ...
    printf("fs.img first 4 bytes = %02x %02x %02x %02x\n",
           fsimg_buf[0],fsimg_buf[1],fsimg_buf[2],fsimg_buf[3]);
}
*/
...
/*
// ------------------------------------------------------------
//  DEBUG   legacyvirtioblk driver  +  fs.img 
// ------------------------------------------------------------
//  * slot  (deviceid==2)
//  * PFN  (version == 1) 
//  * desc1/2  WRITE 
//  * used.idx  printf 
//    - virtio_debug_dump()  PFN/flags/used.idx 
// ------------------------------------------------------------
// 20250605  Public Domain / No warranty
...
#define SECTOR_SIZE   512
#define NUM_DESC      8
#define FSIMG_SECTORS 256   // 128 KiB 
...
//-------------------------------------------------- 4 KiB virtqueue page
#define VQ_PHYS 0x80020000UL
struct virtqueue_page {
    struct virtq_desc  desc[NUM_DESC];
    struct virtq_avail avail;
    uint8_t pad[4096 - (sizeof(struct virtq_desc)*NUM_DESC + sizeof(struct virtq_avail) + sizeof(struct virtq_used))];
    struct virtq_used  used;
} __attribute__((aligned(4096)));
static struct virtqueue_page *vq = (void*)VQ_PHYS;

//-------------------------------------------------- globals
static uint8_t  status_byte __attribute__((aligned(4)));
static uint8_t  dma_buf[SECTOR_SIZE] __attribute__((aligned(512)));
static uint16_t avail_idx = 0;
static uint64_t virtio_base = 0;
...
static void virtio_debug_dump(const char* tag)
{
    printf("[%s] PFN_drv=%x PFN_dev=%x used.idx=%u flags0=%x flags1=%x flags2=%x status=%x\n",
        tag,
        (uint32_t)(VQ_PHYS>>12),
        mr32(virtio_base, REG_QUEUE_PFN),
        vq->used.idx,
        vq->desc[0].flags, vq->desc[1].flags, vq->desc[2].flags,
        mr32(virtio_base, REG_STATUS));
}
...
void virtio_disk_init(void)
{
    ...
    if(!virtio_base){ printf("virtio-blk not found\n"); while(1);}    
    if(mr32(virtio_base,REG_VERSION)!=1){ printf("not legacy mode\n"); while(1); }
...
void virtio_disk_read_sector(uint64_t sector, void *dst)
{
    ...
    vq->desc[0]=(struct virtq_desc){ .addr=(uint64_t)(uintptr_t)&req, .len=sizeof(req), .flags=VIRTQ_DESC_F_NEXT, .next=1 };
    vq->desc[1]=(struct virtq_desc){ .addr=(uint64_t)(uintptr_t)dma_buf, .len=SECTOR_SIZE, .flags=VIRTQ_DESC_F_NEXT|VIRTQ_DESC_F_WRITE, .next=2 };
    vq->desc[2]=(struct virtq_desc){ .addr=(uint64_t)(uintptr_t)&status_byte, .len=1, .flags=VIRTQ_DESC_F_WRITE };

    vq->avail.ring[avail_idx % NUM_DESC]=0;
    __sync_synchronize();
...
/*
// ------------------------------------------------------------
//  Legacy (version = 1) virtioblk driver + fs.img reader (RISCV virt)
//  *  block (deviceid==2) 
//  * PFN  (4KiB)  virtqueue 
//  *  256 
// 20250605  public domain / no warranty
...
// ------------------------------------------------------------
#define REG_MAGIC_VALUE        0x000
#define REG_VERSION            0x004   // 1 = legacy
#define REG_DEVICE_ID          0x008
#define REG_VENDOR_ID          0x00c
#define REG_STATUS             0x070
#define REG_GUEST_PAGE_SIZE    0x028
...
#define REG_QUEUE_NUM          0x034
#define REG_QUEUE_ALIGN        0x038
#define REG_QUEUE_PFN          0x040   // (phys>>12)
#define REG_QUEUE_NOTIFY       0x050
...
// MMIO helpers
// ------------------------------------------------------------
static inline void mmio_w32(uint64_t b,uint32_t o,uint32_t v){ *(volatile uint32_t*)(b+o)=v; }
static inline uint32_t mmio_r32(uint64_t b,uint32_t o){ return *(volatile uint32_t*)(b+o);} 

// ------------------------------------------------------------
// virtioblk  (legacy PFN)
// ------------------------------------------------------------
void virtio_disk_init(void)
{
    // -- slot scan --
    for(int s=0; s<VIRTIO_MAX_SLOTS; ++s){
        uint64_t base = VIRTIO_MMIO_BASE0 + s*VIRTIO_MMIO_STRIDE;
        if(mmio_r32(base, REG_MAGIC_VALUE) != 0x74726976) continue;   // "virt"
        if(mmio_r32(base, REG_DEVICE_ID) == 2){ virtio_base = base; break; }
    }
    if(!virtio_base){ printf("virtioblk not found\n"); while(1); }
    if(mmio_r32(virtio_base, REG_VERSION) != 1){ printf("not legacy mode\n"); while(1); }

    // -- reset & basic status --
    mmio_w32(virtio_base, REG_STATUS, 0);
    mmio_w32(virtio_base, REG_STATUS, STATUS_ACK | STATUS_DRIVER);

    // -- guest page size & queue setup --
    mmio_w32(virtio_base, REG_GUEST_PAGE_SIZE, 4096);

    mmio_w32(virtio_base, REG_QUEUE_SEL, 0);
    uint32_t qsz = mmio_r32(virtio_base, REG_QUEUE_NUM);
    if(qsz < NUM_DESC){ printf("queue too small (%u)\n", qsz); while(1);}  

    mmio_w32(virtio_base, REG_QUEUE_ALIGN, 4096);          // legacy align
    mmio_w32(virtio_base, REG_QUEUE_PFN, (uint32_t)(VQ_PHYS >> 12));

    // --  0  --
    memset(vq, 0, 4096);

    // -- DRIVER_OK --
    mmio_w32(virtio_base, REG_STATUS, STATUS_ACK | STATUS_DRIVER | STATUS_DRIVER_OK);
}
...
void virtio_disk_read_sector(uint64_t sector, void *dst)
{
    ...
    // desc0: req
    vq->desc[0] = (struct virtq_desc){
        .addr  = (uint64_t)(uintptr_t)&req,
        .len   = sizeof(req),
        .flags = VIRTQ_DESC_F_NEXT,
        .next  = 1 };

    // desc1: data buffer (device writes)  WRITE|NEXT
    vq->desc[1] = (struct virtq_desc){
        .addr  = (uint64_t)(uintptr_t)dma_buf,
        .len   = SECTOR_SIZE,
        .flags = VIRTQ_DESC_F_NEXT | VIRTQ_DESC_F_WRITE,
        .next  = 2 };

    // desc2: status (device writes)
    vq->desc[2] = (struct virtq_desc){
        .addr  = (uint64_t)(uintptr_t)&status_byte,
        .len   = 1,
        .flags = VIRTQ_DESC_F_WRITE };
    ...
    // kick
    mmio_w32(virtio_base, REG_QUEUE_NOTIFY, 0);
