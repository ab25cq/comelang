...
uniq come_regex*% come_regex*::initialize(come_regex*% self, char* str)
{
    self.str = string(str);
...
uniq come_regex*% char*::to_regex(char* self)
{
    return new come_regex(self);
}

uniq come_regex*% string::to_regex(char* self)
{
    return new come_regex(self);
}
    ...
    while(true) {
        ...
        if(regex_result >= 0) {
            ...
            if(matchlength == 0) {
                ...
                offset += regex_result + matchlength;
            }
        }
        /// no match ///
        else {
