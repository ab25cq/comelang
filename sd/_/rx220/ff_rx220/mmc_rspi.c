...
static volatile WORD Timer1, Timer2;		/* 1000Hz decrement timer stopped at zero (driven by disk_timerproc()) */
...
static void power_off (void)	/* Disable MMC/SDC interface */
{
	RSPI.SPCR.BYTE = 0;		/* Stop RSPI module */
}
...
)
{
	...
	/* Select the card and wait for it goes ready to accept a command except to stop multiple block read */
	if (cmd != CMD12) {
		deselect();
