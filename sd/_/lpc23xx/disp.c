...
)
{
	...
	n = (0 - biofs % 4) % 4;	/* Align buffer address for fast data transfer */
	buff += n; sz_work -= n;
	do {
...
)
{
	...
	rc = jd_prepare(&jd, tjd_input, work, (uint_fast16_t)sz_work, fp);		/* Prepare to decompress the file */
	if (rc == JDR_OK) {
		for (scale = 0; scale < 3; scale++) {
			if ((jd.width >> scale) <= DISP_XS && (jd.height >> scale) <= DISP_YS) break;
		}
		disp_locate(0, TS_HEIGHT - 1);
		xfprintf(OLED, "%ux%u 1/%u", jd.width, jd.height, 1 << scale);

		rc = jd_decomp(&jd, tjd_output, scale);	/* Start to decompress */
	} else {
		disp_locate(0, 0);
		xfprintf(OLED, "Error: %d", rc);
	}
	uart0_getc();
}
